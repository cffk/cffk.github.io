...
namespace {
  char rcsid[] = "$Id: RandomTest.cpp 6143 2006-04-18 20:35:44Z ckarney $";
}
...
void StepTest() {
  ...
  {
    Random s(vector<unsigned long>(9));
    s.SetCount(-624);
    cout << "AAAAAA " << s();
    cout << " " << s() << endl;
  }
...
long double limits div 16494 ldexp 1074 pow 16383
  ...
  for (size_t i = 0; i < 20; ++i)
    cout << HighPrecTime() << " "
	 << clock() << " "
	 << Random::SeedWord() << endl;

  return;
}

void TestSeeding(Random& r) {
  ...
  nseed.push_back(Random::SeedWord());
  r.Reseed(nseed);
  cout << "Seed: " << r.SeedString() << ", 1st rv: " << r() << endl;
